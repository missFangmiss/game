    ...
    methods: {
      init() {
        ...
        if (this.value.length || this.multi) {
          if (this.range) {
            ...
            this.rangeBegin = [this.year, this.month, this.zero?this.zeroPad(this.day):this.day];
            this.rangeEnd = [yearEnd, monthEnd, this.zero?this.zeroPad(dayEnd):dayEnd];
          } else if (this.multi) {
      ...
      renderOption(year, month, i, playload) {
        ...
        if (this.range) {
          ...
          const options = Object.assign(
            {day: i},
            this.getLunarInfo(year, this.zero?this.zeroPad(month+1):month+1, this.zero?this.zeroPad(i):i),
          ...
          this.zero ? (
            copyRangeBegin[1] = this.zeroPad(copyRangeBegin[1]+1),
            copyRangeEnd[1] = this.zeroPad(copyRangeEnd[1]+1),
            copyRangeBegin[2] = this.zeroPad(copyRangeBegin[2]),
            copyRangeEnd[2] = this.zeroPad(copyRangeEnd[2])
          ) : (copyRangeBegin[1] += 1,copyRangeEnd[1] += 1);
        ...
        if (this.multi) {
          ...
          if (Math.abs(seletSplit[0]) === year && Math.abs(seletSplit[1]) === monthHuman && Math.abs(seletSplit[2]) === i) {
            Object.assign(
              options,
              {day: i, selected: true},
      ...
      select(k1, k2, data, e, monthIndex) {
        ...
        if (this.range) {
          ...
          this.day = day;
          const selectDate = [selectYear, this.zero ? this.zeroPad(selectMonthHuman) : selectMonthHuman, this.zero ? this.zeroPad(selectDay) : selectDay];
      ...
      zeroPad(n) {
        return String(Math.abs(n) < 10 ? `0${Math.abs(n)}` : n);
      },
